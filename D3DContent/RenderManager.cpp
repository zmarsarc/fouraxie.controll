...
//------------------------------------------------------------------------------
HRESULT
CRenderManager::Create(CRenderManager **ppManager)
{
	HRESULT hr = S_OK;

	*ppManager = new CRenderManager();
	IFCOOM(*ppManager);
...
//
//  Member:
//      CRenderManager::EnsureRenderers
//
//  Synopsis:
...
CRenderManager::EnsureRenderers()
{
	...
	if (!m_rgRenderers)
	{
		...
		assert(m_cAdapters);
		m_rgRenderers = new CRender*[m_cAdapters];
		IFCOOM(m_rgRenderers);
		ZeroMemory(m_rgRenderers, m_cAdapters * sizeof(m_rgRenderers[0]));

		for (UINT i = 0; i < m_cAdapters; ++i)
		{
			IFC(CTriangleRender::Create(m_pD3D, m_pD3DEx, m_hwnd, i, &m_rgRenderers[i]));
		}
...
//
//  Member:
//      CRenderManager::EnsureD3DObjects
//
//  Synopsis:
...
//
//  Member:
//      CRenderManager::CleanupInvalidDevices
//
//  Synopsis:
...
//
//  Member:
//      CRenderManager::GetBackBufferNoRef
//
//  Synopsis:
...
//
//  Member:
//      CRenderManager::TestSurfaceSettings
//
//  Synopsis:
...
//
//  Member:
//      CRenderManager::DestroyResources
//
//  Synopsis:
...
//------------------------------------------------------------------------------
void
CRenderManager::SetSize(UINT uWidth, UINT uHeight)
{
...
//------------------------------------------------------------------------------
void
CRenderManager::SetAlpha(bool fUseAlpha)
{
...
//
//  Member:
//      CRenderManager::SetNumDesiredSamples
//
//  Synopsis:
...
//------------------------------------------------------------------------------
void
CRenderManager::SetNumDesiredSamples(UINT uNumSamples)
{
...
//------------------------------------------------------------------------------
void
CRenderManager::SetAdapter(POINT screenSpacePoint)
{
	CleanupInvalidDevices();
